# Swift & Apple Platform Copilot Instructions

When assisting with this codebase, generate clean, safe, and idiomatic Swift 6 code that adheres to modern Apple frameworks and clean architecture principles.

## General Swift & Architecture Guidelines

- **Swift Concurrency:** Default to modern `async/await`, `@MainActor`, and `Task` groups instead of legacy callbacks or `DispatchQueue`.
- **Memory Safety:** Use weak self-references (`[weak self]`) in closures to prevent reference cycles. Ensure value types (`struct`, `enum`) are preferred over reference types (`class`) unless state identity is explicitly needed.
- **Error Handling:** Use typed throwing functions or clear `Result` types. Do not swallow errors in empty `catch` blocks.
- **SwiftUI Best Practices:** Keep views lightweight and declarative. Extract subviews into separate properties or small components. Use `@Observable` (iOS 17+) over `@ObservedObject`/`ObservableObject`.
